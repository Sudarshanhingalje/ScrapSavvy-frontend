...
import LogoutMenu from "../../Components/Common/LogoutMenu";
import Toast from "../../Components/Common/Snackbar";
import ScrapyardSidebar from "../../Components/Layout/ScrapyardSidebar";
...
const ScrapyardProfile = () => {
  ...
  const handlePersonalDetailsChange = async () => {
    ...
    const userProfileId = localStorage.getItem("userId");
    await AuthService.UpdateUserProfile(userProfileId, formData).then(
      (response) => {
        loadUserProfile();
